...
export async function POST(request: Request) {
  try {
    ...
    const roomName = "sintonia-" + Date.now() + "-" + Math.random().toString(36).substring(2, 8);
    ...
    // Gera tokens de acesso pra psicologa e paciente
    const tokenPsicologaResp = await fetch("https://api.daily.co/v1/meeting-tokens", {
      method: "POST",
      headers: {
        Authorization: "Bearer " + dailyApiKey,
        "Content-Type": "application/json",
    ...
    const tokenPacienteResp = await fetch("https://api.daily.co/v1/meeting-tokens", {
      method: "POST",
      headers: {
        Authorization: "Bearer " + dailyApiKey,
        "Content-Type": "application/json",
